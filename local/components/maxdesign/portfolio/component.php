...
if($arParams["SEF_MODE"] == "Y")
{
    ...
    // Смотрим есть ли фильтры в урле
    ...
    if (
        class_exists('\Bitrix\Iblock\Elements\ElementPortfolioFiltersTable')
        && $arParams['SEF_FILTERS']
        && $after
        && count($segments) <= count($arParams['SEF_FILTERS'])
    ) {
        // Достаём фильтры из урла
        $filters = [];
        foreach ($segments as $segment) {
            // Разбиваем каждый сегмент по дефису и сливаем в один массив
            foreach (explode('-', $segment) as $item) {
                $filters[] = $item;
            }
        }

        $rsData = \Bitrix\Iblock\Elements\ElementPortfolioFiltersTable::getList([
            'filter' => [
                'CODE' => $arParams['SEF_FILTERS'],
                'FILTER_AUTOCOMPLETE.VALUE' => false,
                'FILTER_MAIN_PAGE.VALUE' => false,
                'FILTER_PROGRAMMING_VALUE' => $filters,
            ],
            'select' => [
        ...
        while ($arData = $rsData->fetch()) {
            if ($arData['FILTER_PROGRAMMING_VALUE']) {
                $filters[$arData['CODE']][] = $arData['FILTER_PROGRAMMING_VALUE'];
            }
        }
        ...
        // Если прошли проверку, то делаем обработку для типов свойств Список
        if ($isFilterPage && !$b404) {
            // Узнаём типы свойств
            $rsData = \Bitrix\Iblock\PropertyTable::getList([
                'filter' => ['CODE' => $arParams['SEF_FILTERS']],
                'select' => ['CODE', 'PROPERTY_TYPE'],
            ]);

            $propertyTypes = [];
            while ($arData = $rsData->fetch()) {
                $propertyTypes[$arData['CODE']] = $arData['PROPERTY_TYPE'];
            }

            if ($propertyTypes && array_keys($propertyTypes, 'L')) {
                // У всех списков получаем ID значений
                $enumLists = [];
                foreach (array_keys($propertyTypes, 'L') as $id) {
                    $rsData = \CIBlockPropertyEnum::GetList(arFilter: [
                        'PROPERTY_ID' => $id,
                    ]);

                    while ($arData = $rsData->Fetch()) {
                        $enumLists[$arData['PROPERTY_CODE']][$arData['XML_ID']] = $arData['ID'];
                    }
                }

                // Ставим полученные значения фильтрам (только спискам)
                foreach ($filters as $code => $filter) {
                    if ($propertyTypes[$code] !== 'L') {
                        continue;
                    }

                    unset($filters[$code]);
                    foreach ($filter as $value) {
                        $filters[$code][] = $enumLists[$code][$value];
                    }
                }
            }
        }
    ...
    // Меняем логику компонента, если страница на которой мы находимся - смарт фильтры
    if ($isFilterPage) {
        $arVariables['FILTERS'] = $filters;
        $arVariables['FILTERS_RAW'] = $rawFilters;
