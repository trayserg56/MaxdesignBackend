<?
require($_SERVER["DOCUMENT_ROOT"]."/bitrix/header.php");
$APPLICATION->SetPageProperty("title", "Портфолио");
$APPLICATION->SetTitle("filter");
?>
<?
	global $arrFilter;
	$params = $_GET['filter'];
	$params[] = 'dizayn-interera';
    $arrFilter = array(
		"SECTION_CODE" => $params
    );
	?>

<div class="container">
	 <?$APPLICATION->IncludeComponent(
	"bitrix:news.list", 
	"portfolio", 
	array(
		"ADD_ELEMENT_CHAIN" => "N",
		"ADD_SECTIONS_CHAIN" => "Y",
		"AJAX_MODE" => "N",
		"AJAX_OPTION_ADDITIONAL" => "",
		"AJAX_OPTION_HISTORY" => "N",
		"AJAX_OPTION_JUMP" => "N",
		"AJAX_OPTION_STYLE" => "Y",
		"BROWSER_TITLE" => "-",
		"CACHE_FILTER" => "N",
		"CACHE_GROUPS" => "Y",
		"CACHE_TIME" => "36000000",
		"CACHE_TYPE" => "N",
		"CHECK_DATES" => "Y",
		"COMPONENT_TEMPLATE" => "portfolio",
		"DETAIL_ACTIVE_DATE_FORMAT" => "d.m.Y",
		"DETAIL_DISPLAY_BOTTOM_PAGER" => "Y",
		"DETAIL_DISPLAY_TOP_PAGER" => "N",
		"DETAIL_FIELD_CODE" => array(
			0 => "PREVIEW_TEXT",
			1 => "",
		),
		"DETAIL_PAGER_SHOW_ALL" => "Y",
		"DETAIL_PAGER_TEMPLATE" => "",
		"DETAIL_PAGER_TITLE" => "Страница",
		"DETAIL_PROPERTY_CODE" => array(
			0 => "AUTHOR",
			1 => "YEAR",
			2 => "DETAIL",
			3 => "ADDRESS",
			4 => "SQUARE",
			5 => "STATUS",
			6 => "STYLE",
			7 => "TYPE",
			8 => "SLIDER",
			9 => "",
		),
		"DETAIL_SET_CANONICAL_URL" => "N",
		"DISPLAY_BOTTOM_PAGER" => "Y",
		"DISPLAY_DATE" => "Y",
		"DISPLAY_NAME" => "Y",
		"DISPLAY_PICTURE" => "Y",
		"DISPLAY_PREVIEW_TEXT" => "Y",
		"DISPLAY_TOP_PAGER" => "N",
		"FILTER_NAME" => "arrFilter",
		"FILE_404" => "",
		"HIDE_LINK_WHEN_NO_DETAIL" => "N",
		"IBLOCK_ID" => "6",
		"IBLOCK_TYPE" => "portfolio",
		"INCLUDE_IBLOCK_INTO_CHAIN" => "Y",
		"LIST_ACTIVE_DATE_FORMAT" => "d.m.Y",
		"LIST_FIELD_CODE" => array(
			0 => "PREVIEW_PICTURE",
			1 => "",
		),
		"LIST_PROPERTY_CODE" => array(
			0 => "YEAR",
			1 => "SQUARE",
			2 => "STATUS",
			3 => "TYPE",
			4 => "",
		),
		"MESSAGE_404" => "",
		"META_DESCRIPTION" => "-",
		"META_KEYWORDS" => "-",
		"NEWS_COUNT" => "999",
		"PAGER_BASE_LINK_ENABLE" => "N",
		"PAGER_DESC_NUMBERING" => "N",
		"PAGER_DESC_NUMBERING_CACHE_TIME" => "36000",
		"PAGER_SHOW_ALL" => "N",
		"PAGER_SHOW_ALWAYS" => "N",
		"PAGER_TEMPLATE" => ".default",
		"PAGER_TITLE" => "Портфолио",
		"PREVIEW_TRUNCATE_LEN" => "",
		"SEF_FOLDER" => "/portfolio/",
		"SEF_MODE" => "Y",
		"SET_LAST_MODIFIED" => "N",
		"SET_STATUS_404" => "Y",
		"SET_TITLE" => "Y",
		"SHOW_404" => "N",
		"SORT_BY1" => "ACTIVE_FROM",
		"SORT_BY2" => "SORT",
		"SORT_ORDER1" => "DESC",
		"SORT_ORDER2" => "ASC",
		"STRICT_SECTION_CHECK" => "N",
		"USE_CATEGORIES" => "N",
		"USE_FILTER" => "N",
		"USE_PERMISSIONS" => "N",
		"USE_RATING" => "N",
		"USE_RSS" => "N",
		"USE_SEARCH" => "N",
		"USE_SHARE" => "N",
		"FIELD_CODE" => array(
			0 => "",
			1 => "",
		),
		"PROPERTY_CODE" => array(
			0 => "SQUARE",
			1 => "",
		),
		"DETAIL_URL" => "",
		"ACTIVE_DATE_FORMAT" => "d.m.Y",
		"SET_BROWSER_TITLE" => "Y",
		"SET_META_KEYWORDS" => "Y",
		"SET_META_DESCRIPTION" => "Y",
		"PARENT_SECTION" => "",
		"PARENT_SECTION_CODE" => "",
		"INCLUDE_SUBSECTIONS" => "Y"
	),
	false
);?>
</div>
 </section> <section id="contact">
<div class="container">
	<div class="contact-container">
		<div class="contact-adres">
			<h3>ПРИХОДИТЕ К НАМ</h3>
			<p>
				 190020 Санкт-Петербург, <br>
				 Старо-Петергофский проспект 44, <br>
				 офис 310
			</p>
			<p>
 <a href="tel:+79006500050">+7 900 650 00 50</a> <br>
 <a href="tel:+78127866776">+7 812 786 67 76</a>
			</p>
			<p>
				 для общения и заказа <br>
 <a href="mailto:info@maxdesign.pro">info@maxdesign.pro</a>
			</p>
			<p>
				 для предложений по сотрудничеству <br>
 <a href="mailto:offer@maxdesign.pro">offer@maxdesign.pro</a>
			</p>
			<h3>ЧАСЫ РАБОТЫ</h3>
			<p>
				 Пн. - Пт. : 10 - 19 <br>
				 Суббота : 12 - 16
			</p>
		</div>
		<div class="contact-forma">
			<h3>НАПИШИТЕ НАМ</h3>
			<form action="" method="post">
 <input type="text" value="" required="" placeholder="Имя*" size="20" name="name"> <input type="text" value="" required="" placeholder="Телефон*" size="20" name="phone"> <input type="text" required="" value="" placeholder="E-mail*" size="20" name="mail"> <label>Текст сообщения*</label> <textarea></textarea> <button type="submit">Отправить</button>
				<p>
					 *Обязательные для заполнения
				</p>
			</form>
		</div>
	</div>
</div>
 </section>

<?require($_SERVER["DOCUMENT_ROOT"]."/bitrix/footer.php");?>